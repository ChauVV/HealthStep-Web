...
import { DEPLOYER_ADDRESS, DEPLOYER_ABI, USDT_ADDRESS } from '../../config/contracts';
...
function BuyToken() {
  ...
  const fetchEthPrice = async () => {
    try {
      ...
      const price = await tokenDeployer.getETHPrice();
      console.log('ETH Price from Chainlink:', price.toString());
      setEthPrice(price);
    } catch (error) {
  ...
  const calculateTokens = (inputAmount) => {
    ...
    if (paymentMethod === 'ETH') {
      const ethPrice = 1900;
      baseTokens = parseFloat(inputAmount) * ethPrice * HST_PER_USDT;
    } else {
      ...
      <div className="token-sale-content">
        <div className="token-sale-card">
          ...
          <div className="token-sale-form-group">
            ...
            <div className="token-sale-payment-methods">
              <button
                onClick={() => setPaymentMethod('ETH')}
                className={`token-sale-payment-btn ${paymentMethod === 'ETH' ? 'active' : ''}`}
              >
                <FaEthereum size={22} />
                <span>ETH</span>
              </button>
              <button
                ...
                className={`token-sale-payment-btn ${paymentMethod === 'USDT' ? 'active' : ''}`}
              >
                <SiTether size={22} />
                <span>USDT</span>
              </button>
          ...
          <div className="token-sale-form-group">
            ...
            <div className="token-sale-input-group">
              ...
              <div className="token-sale-input-icon">
                {paymentMethod === 'ETH' ? <FaEthereum size={20} /> : <SiTether size={20} />}
              </div>
            </div>
          </div>
